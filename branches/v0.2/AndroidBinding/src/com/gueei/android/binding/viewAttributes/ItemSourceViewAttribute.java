...
import com.gueei.android.binding.collections.ExpandableArrayAdapter;
	...
	private void setArrayAdapter(Object newValue) {
		try {
			...
			if (newValue instanceof IRowModel[]){
				...
				if (ExpandableListView.class.isInstance(getView())){
					String childName = map.get("childItemSource");
					int childLayout = Utility.resolveResource(map.get("childItemTemplate"),
							Binder.getApplication());
					ExpandableArrayAdapter adapter = new ExpandableArrayAdapter(
							Binder.getApplication(),
							newValue.getClass().getComponentType(),
							(Object[]) newValue, spinnerTemplate, itemTemplate, 
							childName, childLayout
							);
					ExpandableListView.class.cast(getView()).setAdapter((ExpandableListAdapter)adapter);
				}else{
