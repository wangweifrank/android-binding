...
public class ExpandableCollectionAdapter extends BaseExpandableListAdapter implements CollectionObserver {
	...
	public ExpandableCollectionAdapter(Context context,
			IObservableCollection<?> collection, int layoutId, int dropDownLayoutId, String childName,
			int childLayoutId)
			throws Exception {
		mChildName = childName;
...
*/
		...
		try {
			if (!mChildAdapters.containsKey(groupPosition)) {
				...
				mChildAdapters.put(groupPosition,
								   Utility.getSimpleAdapter(mContext, childCollection, mChildLayout, mChildLayout,
															null));
	...
	public void onGroupCollapsed(int groupPosition) {
		Object item = mChildAdapters.remove(groupPosition);
	}
