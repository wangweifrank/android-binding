...
public class FilterCursorList {
	...
	public FilterCursorList(Activity activity) {
		mContext = activity;
		CursorSrc = new CursorObservableCollection<FilteredCursorRowModel>(mContext, FilteredCursorRowModel.class);
		mCursorSrc = mContext.getContentResolver()
							 .query(mTrackingUri, new String[]{"_ID", "Name", "detailsCount"}, null, null, null);
	...
	public final CursorObservableCollection<FilteredCursorRowModel> CursorSrc;
	public final Observable<Filter> CursorFilter = new Observable<Filter>(Filter.class, new Filter() {
		...
		protected FilterResults performFiltering(CharSequence constraint) {
			...
			String filterString =
					"lower(Name) like " + S_QUOTES + S_PERC_SIGN + constraint.toString().toLowerCase() + S_PERC_SIGN +
					S_QUOTES;
			filteredCursor = mContext.getContentResolver()
									 .query(mTrackingUri, new String[]{"_ID", "Name", "detailsCount"}, filterString,
											null, null);
